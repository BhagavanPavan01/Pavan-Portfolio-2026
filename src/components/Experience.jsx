...
const ExperienceCard = ({ exp, index, navigate }) => {
    ...
    const desktopInitial = { opacity: 0, x: isEven ? -60 : 60, y: 30 };
    const desktopAnimate = { opacity: 1, x: 0, y: 0 };

    // New impressive 3D scale & slide animation for mobile
    const mobileInitial = { opacity: 0, y: 50, scale: 0.9, rotateX: 15 };
    const mobileAnimate = { opacity: 1, y: 0, scale: 1, rotateX: 0 };
        ...
        <div id={`experience-card-${exp.id}`} className={`scroll-mt-24 relative flex flex-col md:flex-row items-center justify-between w-full mb-12 md:mb-24 group ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
            ...
            <div className={`w-full md:w-[45%] flex relative z-30`}>
                <motion.div
                    ...
                    transition={{ duration: 0.7, type: "spring", stiffness: 90, damping: 20, delay: isMobile ? 0 : 0.1 }}
                    whileHover={{ y: -8 }}
                    onClick={() => {
                        window.history.replaceState(null, '', `/#experience-[${exp.id}]`);
                        navigate(`/experience/${exp.id}`);
                    }}
                    className="w-full bg-gray-900/40 backdrop-blur-xl border border-gray-800 rounded-3xl p-6 md:p-8 cursor-pointer shadow-lg hover:shadow-[0_20px_40px_rgba(168,85,247,0.15)] hover:border-purple-500/50 transition-all duration-300 relative overflow-hidden group/card"
                >
                    ...
                    <div className="h-48 w-full rounded-2xl overflow-hidden mb-6 relative">
                        ...
                        />
                        <div className="absolute top-4 right-4 z-20 bg-gray-900/80 backdrop-blur-md px-3 py-1.5 rounded-full border border-gray-700/50 flex items-center gap-2 shadow-sm">
                            <Calendar className="w-4 h-4 text-purple-400" />
                            <span className="text-xs font-bold text-gray-200 tracking-wide">{exp.duration}</span>
                        </div>
                    </div>

                    {/* Role & Company Header */}
                    <div className="mb-4 relative z-10">
                        <h3 className="text-2xl font-bold text-white mb-2 group-hover/card:text-transparent group-hover/card:bg-clip-text group-hover/card:bg-gradient-to-r group-hover/card:from-purple-400 group-hover/card:to-indigo-500 transition-all duration-300">
                            {exp.role}
                        </h3>
                        <div className="flex items-center gap-2 text-gray-300">
                            <Building2 className="w-4 h-4 text-purple-400" />
                            <h4 className="text-sm md:text-base font-semibold">{exp.company}</h4>
                        </div>
                    </div>

                    {/* Description */}
                    <p className="text-gray-400 text-sm md:text-base leading-relaxed mb-6 relative z-10 font-medium">
                        {exp.desc}
                    </p>
                    ...
                    <div className="flex flex-col gap-5 mt-auto relative z-10">
                        ...
                        <div className="flex items-center text-purple-400 font-bold text-sm bg-gray-800/50 w-fit px-4 py-2 rounded-xl group-hover/card:bg-purple-500 group-hover/card:text-white transition-all duration-300">
                            View Full Impact
            ...
            <div className="container mx-auto px-6 md:px-12 relative z-10">
                ...
                >
                    ...
                    <h2 className="text-4xl md:text-5xl font-black text-white mb-6 tracking-tight">
                        PROFESSIONAL <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-indigo-400 to-cyan-400">EXPERIENCE</span>
                    </h2>

                    <div className="w-24 h-1.5 bg-gradient-to-r from-purple-500 to-indigo-500 mx-auto rounded-full mb-6"></div>

                    <p className="text-gray-400 max-w-3xl mx-auto text-base md:text-lg font-medium leading-relaxed">
